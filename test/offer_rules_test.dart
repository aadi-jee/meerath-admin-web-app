...
void main() {
  OfferStatus status({bool enabled = true, DateTime? now, double discount = 20,
    DateTime? from, DateTime? to}) => OfferRules.status(
      enabled: enabled, price: 40, discount: discount, type: 'percentage',
  ...
  test('Full final day remains active until next Saudi midnight', () {
    expect(status(now: DateTime.utc(2026, 9, 30, 20, 59, 59)), OfferStatus.active);
    expect(status(now: DateTime.utc(2026, 9, 30, 21)), OfferStatus.expired);
  });
  test('Same-day offer is valid', () {
    expect(status(from: DateTime.utc(2026, 9, 8), to: DateTime.utc(2026, 9, 8)), OfferStatus.active);
  });
  ...
  test('Fixed discount cannot exceed price', () {
    expect(OfferRules.validate(price: 22, discount: 23, type: 'fixed',
      from: DateTime.utc(2026,9,1), to: DateTime.utc(2026,9,30)), isNotNull);
  });
  test('Missing and reversed dates are invalid', () {
    expect(OfferRules.validate(price: 22, discount: 10, type: 'percentage', from: null, to: null), isNotNull);
  ...
  test('Legacy ISO and date-only values keep their calendar date', () {
    expect(OfferRules.parseDate('2026-09-08T00:00:00.000'), DateTime.utc(2026,9,8));
