...
class _OfferScreenState extends State<OfferScreen> {
  ...
  Future<void> _change(Future<void> Function() action, String success) async {
    ...
    try {
      ...
      _message(
        refreshed
            ? success
            : 'Change saved. Refresh to load the latest offers.',
      );
    } catch (error) {
  ...
  Future<void> _delete(MenuItemData item) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Remove the offer for ${item.name}? The food item stays in your menu.',
        ),
        actions: [
          TextButton(
    ...
    if (confirmed == true && mounted) {
      await _change(
        () => _repository.removeOffer(item.id),
        'Offer deleted. Menu item kept.',
      );
    }
  }
  ...
  Future<void> _create() async {
    ...
    setState(() => _busy = true);
    try {
      final items = (await _repository.loadDisplayItems())
          .where((i) => !i.hasOffer)
          .toList();
      if (!mounted) return;
      if (items.isEmpty) {
        _message(
          'No items without an offer. Add a menu item first or edit an existing offer.',
        );
        return;
      }
      final item = await showDialog<MenuItemData>(
        context: context,
          ...
          children: [
            for (final item in items)
              SimpleDialogOption(
                ...
                child: Text(
                  '${item.name} · SAR ${item.price.toStringAsFixed(2)}',
                ),
              ),
          ],
        ),
  ...
  OfferStatus _status(MenuItemData item) => OfferRules.status(
    enabled: item.offerActive,
    price: item.price,
    discount: item.offerDiscount,
    type: item.offerType,
    from: item.offerValidFrom,
    to: item.offerValidTo,
  );

  Color _color(OfferStatus status) => switch (status) {
    OfferStatus.active => Colors.green,
    OfferStatus.disabled => Colors.grey,
    OfferStatus.scheduled => Colors.lightBlue,
    OfferStatus.expired => Colors.redAccent,
    OfferStatus.invalid => Colors.orange,
  };

  @override
  Widget build(BuildContext context) {
    final visible = _offers
        .where((i) => _filter == null || _status(i) == _filter)
        .toList();
    final active = _offers
        .where((i) => _status(i) == OfferStatus.active)
        .length;
    ...
    return ListView(
      ...
      children: [
        ...
        const Text(
          'Saudi dates · End date includes the full day. Customer visibility also depends on item and branch availability.',
        ...
        Wrap(
          ...
          children: [
            ...
            for (final status in OfferStatus.values)
              ChoiceChip(
                label: Text(OfferRules.label(status)),
        ...
        if (!locked && _error == null && visible.isEmpty)
          const Padding(
            ...
            child: Text('No offers in this view.'),
          ),
  ...
  Widget _offerCard(MenuItemData item, bool locked) {
    ...
    final discounted = valid && discount != null
        ...
        : null;
    final toggleAllowed =
        item.offerActive || (status != OfferStatus.expired && valid);
    return Card(
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                Text(
                  OfferRules.label(status),
                  style: TextStyle(color: color, fontWeight: FontWeight.bold),
                ),
                Text(
                  '${OfferRules.formatDate(item.offerValidFrom)} → ${OfferRules.formatDate(item.offerValidTo)}',
                ),
                if (discount != null && discount.isFinite)
                  Text(
                    item.offerType == 'percentage'
                        ? '${discount.toStringAsFixed(2)}% off'
                        : 'SAR ${discount.toStringAsFixed(2)} off',
                  ),
                if (discounted != null)
                  Text(
                    'Base SAR ${item.price.toStringAsFixed(2)} → SAR ${discounted.toStringAsFixed(2)} with offer',
                  ),
              ],
            ),
            if (status == OfferStatus.expired || status == OfferStatus.invalid)
              const Padding(
                ...
                child: Text('Edit the offer settings before enabling it.'),
              ),
            Row(
              children: [
                const Expanded(child: Text('Offer enabled')),
                Switch(
                  value: item.offerActive,
                  onChanged: locked || !toggleAllowed
                      ? null
                      : (enabled) => _change(
                          () => _repository.setOfferEnabled(item.id, enabled),
                          enabled ? 'Offer enabled.' : 'Offer disabled.',
                        ),
                ),
              ],
            ),
