...
class MenuRepository {
  ...
  static String errorMessage(Object error) {
    ...
    if (error is StorageException) return 'Image upload failed: ${error.message}';
  ...
  Future<List<Map<String,dynamic>>> loadCategories() async =>
    List<Map<String,dynamic>>.from(await _client.from('categories').select()
      .eq('restaurant_id',restaurantId).order('sort_order').order('id'));

  Future<List<Map<String,dynamic>>> loadSubcategories(List<String> ids) async =>
    ids.isEmpty ? [] : List<Map<String,dynamic>>.from(await _client.from('subcategories')
      .select().inFilter('category_id',ids).order('sort_order').order('id'));

  Future<List<Map<String,dynamic>>> loadBranches() async =>
    List<Map<String,dynamic>>.from(await _client.from('branches').select()
      .eq('restaurant_id',restaurantId).order('name'));

  Future<List<Map<String,dynamic>>> loadMenuItems() async =>
    List<Map<String,dynamic>>.from(await _client.from('menu_items')
      .select('*, categories(name_en), subcategories(name_en)')
      .eq('restaurant_id',restaurantId).neq('status','archived').order('sort_order').order('id'));

  Future<Map<String,dynamic>> loadItem(String id) async =>
    await _client.from('menu_items').select().eq('restaurant_id',restaurantId).eq('id',id).single();

  Future<List<Map<String,dynamic>>> loadSchedules(String id) async =>
    List<Map<String,dynamic>>.from(await _client.from('menu_item_schedules').select()
      .eq('menu_item_id',id).order('day_of_week'));

  Future<List<String>> loadItemBranches(String id) async {
    final rows = await _client.from('branch_menu_items').select('branch_id')
      .eq('menu_item_id',id).eq('is_available',true);
    return rows.map((r) => r['branch_id'] as String).toList();
  }

  Future<String> loadNotes(String id) async {
    final row = await _client.from('menu_item_private').select('notes')
      .eq('menu_item_id',id).maybeSingle();
  ...
  Future<void> _updateItem(String id, Map<String,dynamic> data) async {
    await _client.from('menu_items').update({...data,'updated_at':DateTime.now().toUtc().toIso8601String()})
      .eq('restaurant_id',restaurantId).eq('id',id).select('id').single();
  }

  Future<String> saveMenuItem(Map<String,dynamic> values, {
  ...
  Future<void> duplicateMenuItem(String id) async {
    ...
    await saveMenuItem({...row,'id':newId(),'name_en':'${row['name_en']} Copy',
      'status':'draft','is_available':true,'is_featured':false},
      schedules: schedules.isEmpty ? defaultSchedules() : schedules,
      branchIds:branches.isEmpty ? (await loadBranches()).where((b)=>b['is_active']==true)
  ...
  Future<void> saveGroup({String? id, String? parentId,
    ...
    if(id==null) {
      ...
      await _client.from(table).update(values).eq('id',id)
        .eq(parentId==null?'restaurant_id':'category_id',parentId??restaurantId).select('id').single();
    }
  }

  Future<void> setGroupActive(String id,bool active,{String? parentId}) async {
    await _client.from(parentId==null?'categories':'subcategories')
      .update({'is_active':active,'updated_at':DateTime.now().toUtc().toIso8601String()})
      .eq('id',id).eq(parentId==null?'restaurant_id':'category_id',parentId??restaurantId)
      .select('id').single();
  }

  Future<void> deleteGroup(String id,{required bool subcategory}) async {
    await _client.rpc('delete_admin_menu_group',params:{
      'target_restaurant_id':restaurantId,'group_id':id,'is_subcategory':subcategory});
  }
  ...
  Future<String> uploadImage(Uint8List bytes,String name) async {
    ...
    final type=switch(extension){'jpg'||'jpeg'=>'image/jpeg','png'=>'image/png','webp'=>'image/webp',_=>null};
    ...
    await _client.storage.from(imageBucket).uploadBinary(path,bytes,
  ...
  Future<List<MenuItemData>> loadDisplayItems() async {
    ...
    return rows.map((r){
      ...
      return MenuItemData(id:r['id'] as String,name:r['name_en'] as String,
        nameAr:r['name_ar'] as String? ?? '',description:r['description_en'] as String? ?? '',
        descriptionAr:r['description_ar'] as String? ?? '',longDescription:r['description_en'] as String? ?? '',
        categoryId:r['category_id'] as String,subcategoryId:r['subcategory_id'] as String?,
        category:r['categories']?['name_en'] as String? ?? '',
        subcategory:r['subcategories']?['name_en'] as String? ?? '',
        price:(r['base_price'] as num).toDouble(),compareAtPrice:(a['compare_at_price'] as num?)?.toDouble(),
        availableAt:'Configured branches',portions:0,available:available,featured:r['is_featured']==true,
