...
class OfferRules {
  static DateTime saudiDate([DateTime? instant]) {
    final local = (instant ?? DateTime.now()).toUtc().add(
  ...
  static String? validate({
    ...
    if (discount == null || !discount.isFinite || discount <= 0) {
      return 'Enter a finite discount greater than zero.';
    }
    if (type == 'percentage' && discount > 100) {
      return 'Percentage discount cannot exceed 100%.';
    }
    if (type == 'fixed' && discount > price) {
      return 'Fixed discount cannot exceed the item price.';
    }
    if (from == null || to == null) return 'Select both offer dates.';
    if (dateOnly(from).isAfter(dateOnly(to))) {
      return 'End date cannot be before start date.';
    }
    return null;
  }

  static OfferStatus status({
